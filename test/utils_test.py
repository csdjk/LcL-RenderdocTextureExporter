...
class ControllerDataStats:
    ...
    def count_texture_resolutions_in_range_by_actionid(self, start_action_id, end_action_id, isOutput=False):
        """
        统计指定 action_id 范围内所有输入或输出纹理的不同分辨率数量
        :param start_action_id: 起始 action_id
        :param end_action_id: 结束 action_id
        :param isOutput: True 统计输出纹理，False 统计输入纹理
        :return: dict {(width, height): 数量}
        """
        res_counter = {}
        for aid in range(start_action_id, end_action_id + 1):
            action = self.get_action_by_actionid(aid)
            if not action:
                continue
            textures = action.outputsTextures if isOutput else action.inputsTextures
            for tex in textures:
...
def sampleCode(controller):
    ...
    resources = controller.GetResources()
    stats = ControllerDataStats(controller,pyrenderdoc)
    texture_resolutions = stats.count_texture_resolutions_in_range_by_actionid(10, 11)
    msg = "\n纹理分辨率统计:\n"
    for res, count in texture_resolutions.items():
        msg += f"{res}: {count}张\n"
    print(msg)        
